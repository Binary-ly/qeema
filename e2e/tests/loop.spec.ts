...
async function observationCount(
    ...
    const response = await request.get(`/api/v1/locations/${fixture.locationSlug}/index/${date}`);
    ...
    const item = snapshot.data.items.find((i: { item: { code: string } }) => i.item.code === fixture.itemCode);
...
test.describe('the closed loop', () => {
    ...
    test('publishes the aggregate without exposing who reported it', async ({ request }) => {
        ...
        for (const leak of ['reporter', 'reporter_ref', 'submission_id', 'device']) {
            expect(body, `the published snapshot mentions ${leak}`).not.toContain(leak);
        }
    });
});
...
test.describe('the platform reports on itself', () => {
    ...
    test('never publishes how thin the screening currently is', async ({ request }) => {
        const health = await (await request.get('/api/v1/health')).json();
        ...
        for (const [name, check] of Object.entries<Record<string, unknown>>(health.pipeline)) {
